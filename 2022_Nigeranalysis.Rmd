...
knitr::opts_knit$set(root.dir ="/Users/drewschwartz/Box Sync/2023_LancetMicrobe_NigerSAMAbxMicrobiome")
```
...
metadata <- read.delim(sep=',',"230717_Nigermetadata.csv",header=TRUE,stringsAsFactors = F)
...
pairwise.wilcox.test(DF_widemeta3time0$shannon, DF_widemeta3time0$Treatment)
#p=0.19
pairwise.wilcox.test(DF_widemeta3time1$shannon, DF_widemeta3time1$Treatment)
#p=.00085
pairwise.wilcox.test(DF_widemeta3time4$shannon, DF_widemeta3time4$Treatment)
#p=0.5
pairwise.wilcox.test(DF_widemeta3time8$shannon, DF_widemeta3time8$Treatment)
#p=0.98
pairwise.wilcox.test(DF_widemeta3time12$shannon, DF_widemeta3time12$Treatment)
#p=1.0

pairwise.wilcox.test(DF_widemeta3time104$shannon, DF_widemeta3time104$Treatment, p.adjust.method = "none")
#           Amoxicillin Healthy Placebo
#Healthy    0.0019      -       -      
#Placebo    0.1486      0.3628  -      
#SAMcontrol 0.0448      0.4929  0.1518

pairwise.wilcox.test(DF_widemeta3time104$shannon, DF_widemeta3time104$Treatment, p.adjust.method = "BH")
#           Amoxicillin Healthy Placebo
#Healthy    0.011       -       -      
#Placebo    0.228       0.435   -      
#SAMcontrol 0.134       0.493   0.228  
...
Tukshanndescription
 # Description           lsmean    SE  df lower.CL upper.CL
 # Healthy_NA_wk104        1.98 0.112 299     1.76     2.20
 # SAM_Amoxicillin_wk0     2.18 0.108 408     1.97     2.39
 # SAM_Amoxicillin_wk1     1.55 0.104 398     1.35     1.75
 # SAM_Amoxicillin_wk104   2.61 0.130 405     2.35     2.87
 # SAM_Amoxicillin_wk12    2.11 0.101 393     1.91     2.31
 # SAM_Amoxicillin_wk4     2.00 0.108 407     1.79     2.22
 # SAM_Amoxicillin_wk8     2.10 0.115 414     1.88     2.33
 # SAM_Placebo_wk0         1.71 0.113 413     1.49     1.93
 # SAM_Placebo_wk1         2.05 0.111 412     1.83     2.27
 # SAM_Placebo_wk104       1.99 0.166 371     1.66     2.32
 # SAM_Placebo_wk12        1.94 0.110 411     1.72     2.15
 # SAM_Placebo_wk4         2.04 0.099 398     1.85     2.24
 # SAM_Placebo_wk8         2.00 0.120 412     1.76     2.23
 # SAMcontrol_NA_wk104     1.80 0.282 299     1.24     2.35
...
pairwise.wilcox.test(DF_widemeta3time0$richness, DF_widemeta3time0$Treatment)
#p=0.25
pairwise.wilcox.test(DF_widemeta3time1$richness, DF_widemeta3time1$Treatment)
#p=0.02
pairwise.wilcox.test(DF_widemeta3time4$richness, DF_widemeta3time4$Treatment)
#p=0.76
pairwise.wilcox.test(DF_widemeta3time8$richness, DF_widemeta3time8$Treatment)
#p=0.74
pairwise.wilcox.test(DF_widemeta3time12$richness, DF_widemeta3time12$Treatment)
#p=.098

pairwise.wilcox.test(DF_widemeta3time104$richness, DF_widemeta3time104$Treatment, p.adjust.method = "none")
#           Amoxicillin Healthy Placebo
#Healthy    0.00087     -       -      
#Placebo    0.00433     0.52350 -      
#SAMcontrol 0.00658     0.50459 0.20267

pairwise.wilcox.test(DF_widemeta3time104$richness, DF_widemeta3time104$Treatment, p.adjust.method = "BH")
#           Amoxicillin Healthy Placebo
#Healthy    0.0052      -       -      
#Placebo    0.0130      0.5235  -      
#SAMcontrol 0.0132      0.5235  0.3040 

RichAOVDF_widemeta3time104<-aov(richness~Treatment, data=DF_widemeta3time104)
TukeyHSD(RichAOVDF_widemeta3time104, "Treatment", ordered= FALSE)

#Fit: aov(formula = richness ~ Treatment, data = DF_widemeta3time104); TUKEYHSD

#$Treatment
#                             diff       lwr        upr     p adj
#Healthy-Amoxicillin    -17.619617 -28.94753 -6.2917030 0.0006173
#Placebo-Amoxicillin    -14.909091 -29.70114 -0.1170374 0.0474855
#SAMcontrol-Amoxicillin -23.742424 -43.21707 -4.2677808 0.0105273
#Placebo-Healthy          2.710526 -10.87571 16.2967599 0.9529882
#SAMcontrol-Healthy      -6.122807 -24.69813 12.4525162 0.8222640
#SAMcontrol-Placebo      -8.833333 -29.70259 12.0359226 0.6832101
...
Tukrichdescription
 # Description           lsmean   SE  df lower.CL upper.CL
 # Healthy_NA_wk104        41.3 2.76 269     35.9     46.7
 # SAM_Amoxicillin_wk0     37.6 2.57 398     32.6     42.7
 # SAM_Amoxicillin_wk1     27.7 2.47 384     22.9     32.6
 # SAM_Amoxicillin_wk104   60.7 3.02 411     54.7     66.6
 # SAM_Amoxicillin_wk12    40.3 2.43 376     35.5     45.1
 # SAM_Amoxicillin_wk4     35.7 2.56 397     30.7     40.7
 # SAM_Amoxicillin_wk8     36.7 2.69 411     31.4     42.0
 # SAM_Placebo_wk0         27.8 2.65 410     22.6     33.0
 # SAM_Placebo_wk1         35.2 2.61 409     30.1     40.3
 # SAM_Placebo_wk104       36.9 3.78 369     29.4     44.3
 # SAM_Placebo_wk12        31.8 2.59 406     26.7     36.8
 # SAM_Placebo_wk4         35.1 2.36 387     30.5     39.8
 # SAM_Placebo_wk8         34.7 2.79 414     29.2     40.2
 # SAMcontrol_NA_wk104     35.2 6.95 269     21.5     48.8
# contrast TUKEY                                estimate   SE  df t.ratio p.value
...
#11
summary(time0amox$BAGE)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #   6.00    8.00   11.00   16.66   21.50   49.00 
IQR(time0amox$BAGE)
#13.5
...
#11.5
summary(time0placebo$BAGE)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #   6.00    8.00   11.50   14.09   19.50   40.00 
...
IQR(time0amox$BMUAC)
#0.4
summary(time0amox$BMUAC)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    9.20   11.00   11.30   11.16   11.40   12.10 
median(time0placebo$BMUAC)
#11.15
IQR(time0placebo$BMUAC)
#0.6
summary(time0placebo$BMUAC)
   # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
   # 9.50   10.80   11.15   11.14   11.40   12.30 
...
IQR(time0amox$BWFHZ)
#0.595
summary(time0amox$BWFHZ)
 #  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # -4.730  -3.380  -3.170  -3.140  -2.785  -1.790 
median(time0placebo$BWFHZ)
#-3.115
IQR(time0placebo$BWFHZ)
#0.835
summary(time0placebo$BWFHZ)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # -4.170  -3.482  -3.115  -3.046  -2.647  -1.240
...
#13
summary(time1amox$visAge)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #   6.25    8.25   12.25   17.02   21.25   49.25 
time1amox$visAge[order(time1amox$visAge)]
median(time1placebo$visAge)
#12.25
IQR(time1placebo$visAge)
#11
summary(time1placebo$visAge)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    7.25    8.25   12.25   14.64   19.25   40.25 
time1placebo$visAge[order(time1placebo$visAge)]
...
#0.65
summary(time1amox$visMUAC)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #   9.40   11.35   11.70   11.61   12.00   13.00 
median(time1placebo$visMUAC)
#11.4
IQR(time1placebo$visMUAC)
#0.8
summary(time1placebo$visMUAC)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   10.30   11.00   11.40   11.42   11.80   12.60 
...
#0.3
summary(time1amox$visdelMUAC)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #  0.000   0.300   0.400   0.441   0.600   1.200 
median(time1placebo$visdelMUAC)
#0.3
IQR(time1placebo$visdelMUAC)
#0.4
summary(time1placebo$visdelMUAC)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -0.3000  0.0000  0.3000  0.3091  0.4000  1.3000 
...
#0.795
summary(time1amox$visWHZ)
 #  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # -4.050  -2.490  -2.130  -1.985  -1.695   1.050 
median(time1placebo$visWHZ)
#-2.19
IQR(time1placebo$visWHZ)
#1.29
summary(time1placebo$visWHZ)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # -3.940  -3.060  -2.190  -2.261  -1.770   0.540 
...
#0.595
summary(time1amox$visdelWHZ)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #  0.160   0.795   1.060   1.163   1.390   5.780 
median(time1placebo$visdelWHZ)
#0.63
IQR(time1placebo$visdelWHZ)
#0.97
summary(time1placebo$visdelWHZ)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -0.5300  0.1700  0.6300  0.6739  1.1400  2.0100 
...
#11
summary(time4amox$visAge)
  # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
  #  7.00    9.00   12.00   16.03   20.00   50.00
time4amox$visAge[order(time4amox$visAge)]
median(time4placebo$visAge)
#12
IQR(time4placebo$visAge)
#12
summary(time4placebo$visAge)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    7.00    9.00   12.00   16.05   21.00   41.00 
time4placebo$visAge[order(time4placebo$visAge)]
...
#0.55
summary(time4amox$visMUAC)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #   9.40   11.75   12.00   11.94   12.30   13.60 
median(time4placebo$visMUAC)
#11.6
IQR(time4placebo$visMUAC)
#1.15
summary(time4placebo$visMUAC)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #  10.30   11.20   11.60   11.78   12.35   13.80 
...
#1.155
summary(time4amox$visWHZ)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # -3.820  -1.930  -1.430  -1.503  -0.775   0.070
median(time4placebo$visWHZ)
#-1.6
IQR(time4placebo$visWHZ)
#1.25
summary(time4placebo$visWHZ)
 #  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # -3.830  -2.265  -1.600  -1.577  -1.015   1.670
...
#0.5
summary(time4amox$visdelMUAC)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   -0.20    0.50    0.70    0.78    1.00    1.70 
median(time4placebo$visdelMUAC)
#0.6
IQR(time4placebo$visdelMUAC)
#0.85
summary(time4placebo$visdelMUAC)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#  -0.500   0.200   0.600   0.686   1.050   1.600 
...
#0.815
summary(time4amox$visdelWHZ)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # -0.500   1.135   1.460   1.533   1.950   2.880 
median(time4placebo$visdelWHZ)
#1.28
IQR(time4placebo$visdelWHZ)
#1.195
summary(time4placebo$visdelWHZ)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # -0.740   0.835   1.280   1.390   2.030   4.720
...
#12
summary(time8amox$visAge)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#     8.0    12.0    14.0    19.2    24.0    51.0 
time8amox$visAge[order(time8amox$visAge)]
median(time8placebo$visAge)
#13
IQR(time8placebo$visAge)
#13
summary(time8placebo$visAge)
  # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
  #  8.00   10.00   13.00   17.19   23.00   42.00 
time8placebo$visAge[order(time8placebo$visAge)]
...
#0.775
summary(time8amox$visMUAC)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   11.80   12.22   12.50   12.66   13.00   13.70
median(time8placebo$visMUAC, na.rm = TRUE)
#11.9
IQR(time8placebo$visMUAC, na.rm = TRUE)
#0.4
summary(time8placebo$visMUAC)
   # Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
   # 9.80   11.70   11.90   11.87   12.10   13.30       1 
...
#0.9075
summary(time8amox$visWHZ)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -1.9800 -1.2525 -1.0200 -0.8763 -0.3450  0.3900 
median(time8placebo$visWHZ, na.rm = TRUE)
#-1.415
IQR(time8placebo$visWHZ, na.rm = TRUE)
#1.05
summary(time8placebo$visWHZ)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#  -3.860  -1.645  -1.415  -1.240  -0.595   2.610       1 
...
#0.6
summary(time8amox$visdelMUAC)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   0.500   1.000   1.150   1.277   1.600   2.200
median(time8placebo$visdelMUAC, na.rm = TRUE)
#0.9
IQR(time8placebo$visdelMUAC, na.rm = TRUE)
#0.725
summary(time8placebo$visdelMUAC)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
# -0.6000  0.6250  0.9000  0.9077  1.3500  2.4000       1 
...
#0.9225
summary(time8amox$visdelWHZ)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #  0.650   1.798   2.275   2.204   2.720   3.170
median(time8placebo$visdelWHZ, na.rm = TRUE)
#1.67
IQR(time8placebo$visdelWHZ, na.rm = TRUE)
#1.63
summary(time8placebo$visdelWHZ)
 #   Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
 # -0.410   0.855   1.670   1.661   2.485   5.160       1 
...
#13
summary(time12amox$visAge)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    9.00   12.00   15.00   19.56   25.00   52.00 
time12amox$visAge[order(time12amox$visAge)]
median(time12placebo$visAge)
#13
IQR(time12placebo$visAge)
#7.5
summary(time12placebo$visAge)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    9.00   11.00   13.00   16.44   18.50   51.00 
time12placebo$visAge[order(time12placebo$visAge)]
...
#1.2
summary(time12amox$visMUAC)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    11.2    12.2    12.5    12.7    13.4    14.5 
median(time12placebo$visMUAC)
#12.3
IQR(time12placebo$visMUAC)
#0.775
summary(time12placebo$visMUAC)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   11.00   11.82   12.30   12.28   12.60   13.60
...
#1.27
summary(time12amox$visWHZ)
#  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -2.4600 -1.4800 -0.6600 -0.7085 -0.2100  1.5600
median(time12placebo$visWHZ)
#-1.065
IQR(time12placebo$visWHZ)
#1.0125
summary(time12placebo$visWHZ)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -2.9000 -1.3900 -1.0650 -0.8588 -0.3775  1.6800 
...
#1
summary(time12amox$visdelMUAC)
# min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#  -0.100   0.900   1.300   1.388   1.900   3.500
median(time12placebo$visdelMUAC)
#1.1
IQR(time12placebo$visdelMUAC)
#0.75
summary(time12placebo$visdelMUAC)
 #   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # -0.100   0.725   1.100   1.174   1.475   2.800 
...
#1.41
summary(time12amox$visdelWHZ)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    0.72    1.65    2.28    2.36    3.06    4.05 
median(time12placebo$visdelWHZ)
#1.965
IQR(time12placebo$visdelWHZ)
#1.1425
summary(time12placebo$visdelWHZ)
 #   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # -1.120   1.435   1.965   1.946   2.578   4.060 
...
#36
summary(time104amox$SAGE)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
 #  30.00   32.00   36.00   41.14   46.00   76.00       1 
time104amox$SAGE[order(time104amox$SAGE)]
...
#9.25
summary(time104placebo$SAGE)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#   32.00   34.00   38.50   38.92   43.25   48.00       1 
time104placebo$SAGE[order(time104placebo$SAGE)]
median(time104healthy$SAGE)
#12
IQR(time104healthy$SAGE)
#13
summary(time104healthy$SAGE)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    6.00    9.00   12.00   16.68   22.00   50.00 
time104healthy$SAGE[order(time104healthy$SAGE)]
median(time104SAM$SAGE)
#10.5
IQR(time104SAM$SAGE)
#5.5
summary(time104SAM$SAGE)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    6.00    8.50   10.50   12.33   14.00   24.00 
time104SAM$SAGE[order(time104SAM$SAGE)]
table(time104amox$BSEX)
#10M and 12F (48%M)
table(time104placebo$BSEX)
#9M and 4F (67%M)
...
#1.4
summary(time104amox$visdelMUAC)
  # Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
  # 1.800   2.800   3.700   3.838   4.200   8.600       1 
median(time104placebo$visdelMUAC,na.rm = TRUE)
#3.45
IQR(time104placebo$visdelMUAC, na.rm = TRUE)
#1.375
summary(time104placebo$visdelMUAC)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#   1.600   2.625   3.450   3.400   4.000   6.000       1 
...
#1.49
summary(time104amox$visdelWHZ)
  # Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
  # 1.850   2.330   3.290   3.208   3.820   5.310       1 
median(time104placebo$visdelWHZ, na.rm = TRUE)
#3.08
IQR(time104placebo$visdelWHZ, na.rm = TRUE)
#1.6175
summary(time104placebo$visdelWHZ)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#   1.240   1.920   3.080   2.834   3.538   4.560       1 
pairwise.wilcox.test(metadatatime104amoxpla$visMUAC, metadatatime104amoxpla$Treatment)
#p=0.22
pairwise.wilcox.test(metadatatime104amoxpla$visWHZ, metadatatime104amoxpla$Treatment)
#p=0.49
...
#1.4
summary(time104amox$SMUAC)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#   13.00   14.00   14.60   14.99   15.40   19.60       1 
median(time104placebo$SMUAC, na.rm = TRUE)
#14
IQR(time104placebo$SMUAC, na.rm = TRUE)
#0.9
summary(time104placebo$SMUAC)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#   13.00   13.75   14.00   14.40   14.65   16.80       1 
median(time104healthy$SMUAC)
#13.9
IQR(time104healthy$SMUAC)
#1.35
summary(time104healthy$SMUAC)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   12.60   13.45   13.90   14.18   14.80   16.40
median(time104SAM$SMUAC)
#11.65
IQR(time104SAM$SMUAC)
#0.625
summary(time104SAM$SMUAC)
# min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   11.20   11.45   11.65   11.73   12.07   12.30 
...
#1.8
summary(time104amox$SZWFL)
#   Min.   1st Qu.    Median      Mean   3rd Qu.      Max.      NA's 
# -1.370000 -0.920000 -0.210000  0.003333  0.880000  2.370000         1 
median(time104placebo$SZWFL, na.rm = TRUE)
#-0.23
IQR(time104placebo$SZWFL, na.rm = TRUE)
#1.415
summary(time104placebo$SZWFL)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
# -2.2800 -1.1225 -0.2300 -0.3700  0.2925  1.4700       1 
median(time104healthy$SZWFL, na.rm = TRUE)
#-0.99
IQR(time104healthy$SZWFL, na.rm = TRUE)
#1.845
summary(time104healthy$SZWFL)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -2.2700 -1.3475 -0.9900 -0.5287  0.4975  1.6600
median(time104SAM$SZWFL, na.rm = TRUE)
#-3.23
IQR(time104SAM$SZWFL, na.rm = TRUE)
#0.16
summary(time104SAM$SZWFL)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#  -3.550  -3.303  -3.230  -3.223  -3.143  -2.890 
...
Specieswidemeta<-Specieswidemeta[-c(1:213)]
...
#Just placebo and Amox over time
Specieswidemeta<-join(metadata, Specieswide)
amoxpla<-subset(Specieswidemeta, Treatment=="Amoxicillin"|Treatment=="Placebo")
.rowNamesDF(amoxpla)<-amoxpla$Sample
amoxpla<-amoxpla[-c(1:213)]

Brayamoxpla <-vegdist(amoxpla, method = "bray")
pco.brayDFamoxpla<-pco(Brayamoxpla, k=4)
Brayamoxpla<-as.data.frame(pco.brayDFamoxpla$points)
BrayDFamoxpla<-as.data.frame(pco.brayDFamoxpla$points)

BrayDFamoxpla$Sample<-row.names(BrayDFamoxpla)
BrayDFamoxpla<-join(BrayDFamoxpla, metadata, by = "Sample")

### Eigenvalues for amox pla data##
EigenBrayamoxpla<- rbind(SD = sqrt(pco.brayDFamoxpla$eig),Proportion = pco.brayDFamoxpla$eig/sum(pco.brayDFamoxpla$eig),Cumulative = cumsum(pco.brayDFamoxpla$eig)/sum(pco.brayDFamoxpla$eig))
#V1 = 23.3 and V2 = 11.7
BrayDF$Sample=NULL

#Reorder levels
BrayDFamoxpla$Description=factor(BrayDFamoxpla$Description, levels = c("SAM_Amoxicillin_wk0","SAM_Amoxicillin_wk1","SAM_Amoxicillin_wk4","SAM_Amoxicillin_wk8","SAM_Amoxicillin_wk12","SAM_Amoxicillin_wk104","SAM_Placebo_wk0","SAM_Placebo_wk1","SAM_Placebo_wk4","SAM_Placebo_wk8","SAM_Placebo_wk12","SAM_Placebo_wk104"))


a<-ggplot(BrayDFamoxpla, aes(x=V1, y=V2, color=Description, shape=as.factor(Week_of_follow_up))) + geom_point()+
  theme_bw()+theme(legend.text=element_text(size=12, face="bold"),legend.title=element_text(size=14, face="bold"),
        panel.grid.minor = element_blank(), panel.border = element_rect(colour = "black"), 
        axis.text.x= element_text(size=12,angle=45,hjust=1,vjust=1,face="bold"),
        axis.text.y= element_text(size=12,face="bold"))+ scale_color_manual(values=c("SAM_Amoxicillin_wk0"="#ffffb2","SAM_Amoxicillin_wk1"="#fed976","SAM_Amoxicillin_wk4"="#feb24c","SAM_Amoxicillin_wk8"="#fd8d3c","SAM_Amoxicillin_wk12"="#f03b20","SAM_Amoxicillin_wk104"="#E31A1C","SAM_Placebo_wk0"="#f1eef6","SAM_Placebo_wk1"="#d0d1e6","SAM_Placebo_wk4"="#a6bddb","SAM_Placebo_wk8"="#74a9cf","SAM_Placebo_wk12"="#2b8cbe","SAM_Placebo_wk104"="#1F78B4"))+
  ylab("PCO2 (11.7%)")+
  xlab("PCO1 (23.3%)") 
ggMarginal(a,type="boxplot", groupColour = TRUE, groupFill = TRUE, size = 2)
ggsave(file="2021Figs/230913_AmoxPlaBCplotplusggmarginalshape.pdf")

adonis2(Brayamoxpla~Description, data = BrayDFamoxpla, permutations = 999, method ="bray", na.rm=TRUE)
#              Df SumOfSqs      R2     F Pr(>F)
# Description  11 11134717 0.95199 670.6  0.192
# Residual    372   561520 0.04801             
# Total       383 11696237 1.00000    

ggplot(BrayDFamoxpla, aes(x=Description, y=V1, color=Treatment))+geom_boxplot(outlier.shape = NA) +geom_point(position = position_jitterdodge(jitter.width = 0.1)) +ggtitle("BC V1")+xlab("Description")+ylab("PCoA V1") + theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme(axis.text.x = element_text(size=16), axis.title.x=element_text(size=20), axis.title.y = element_text(size=20), axis.text.y = element_text(size=16), axis.title = element_text(size=24))+scale_color_manual(values = c("Amoxicillin" = "#E31A1C", "Placebo" = "#1F78B4"))+scale_x_discrete(limits=c("SAM_Amoxicillin_wk0","SAM_Amoxicillin_wk1","SAM_Amoxicillin_wk4","SAM_Amoxicillin_wk8","SAM_Amoxicillin_wk12","SAM_Amoxicillin_wk104","SAM_Placebo_wk0","SAM_Placebo_wk1","SAM_Placebo_wk4","SAM_Placebo_wk8","SAM_Placebo_wk12","SAM_Placebo_wk104"))
ggsave(file="2021Figs/230122_AmoxPlaBCV1.pdf")
pairwise.wilcox.test(BrayDFamoxpla$V1, BrayDFamoxpla$Description, p.adjust.method = "BH")

#                       SAM_Amoxicillin_wk0 SAM_Amoxicillin_wk1 SAM_Amoxicillin_wk104 SAM_Amoxicillin_wk12 SAM_Amoxicillin_wk4 SAM_Amoxicillin_wk8 SAM_Placebo_wk0 SAM_Placebo_wk1 SAM_Placebo_wk104
# SAM_Amoxicillin_wk1   0.7493              -                   -                     -                    -                   -                   -               -               -                
# SAM_Amoxicillin_wk104 0.0020              0.0020              -                     -                    -                   -                   -               -               -                
# SAM_Amoxicillin_wk12  0.8146              0.9483              0.0067                -                    -                   -                   -               -               -                
# SAM_Amoxicillin_wk4   0.8271              0.8271              0.0067                0.8271               -                   -                   -               -               -                
# SAM_Amoxicillin_wk8   0.8271              0.9483              0.0032                0.8638               0.8911              -                   -               -               -                
# SAM_Placebo_wk0       0.9950              0.8146              0.0020                0.8146               0.8614              0.8271              -               -               -                
# SAM_Placebo_wk1       0.8271              0.9950              0.0089                0.8983               0.8638              0.9950              0.8271          -               -                
# SAM_Placebo_wk104     0.7493              0.8271              0.3172                0.8271               0.8271              0.8271              0.8271          0.8271          -                
# SAM_Placebo_wk12      0.8271              0.8271              0.0012                0.8271               0.9950              0.8638              0.8662          0.8271          0.8271           
# SAM_Placebo_wk4       0.8271              0.8271              0.0020                0.8271               0.9950              0.8828              0.8271          0.8638          0.8271           
# SAM_Placebo_wk8       0.8271              0.8953              0.0032                0.8828               0.8638              0.9950              0.8271          0.9950          0.8271           
#                       SAM_Placebo_wk12 SAM_Placebo_wk4
# SAM_Amoxicillin_wk1   -                -              
# SAM_Amoxicillin_wk104 -                -              
# SAM_Amoxicillin_wk12  -                -              
# SAM_Amoxicillin_wk4   -                -              
# SAM_Amoxicillin_wk8   -                -              
# SAM_Placebo_wk0       -                -              
...
# SAM_Placebo_wk104     -                -              
# SAM_Placebo_wk12      -                -              
# SAM_Placebo_wk4       0.9950           -              
# SAM_Placebo_wk8       0.8271           0.8271         

# P value adjustment method: BH 

ggplot(BrayDFamoxpla, aes(x=Description, y=V2, color=Treatment))+geom_boxplot(outlier.shape = NA) +geom_point(position = position_jitterdodge(jitter.width = 0.1)) +ggtitle("BC V1")+xlab("Description")+ylab("PCoA V1") + theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme(axis.text.x = element_text(size=16), axis.title.x=element_text(size=20), axis.title.y = element_text(size=20), axis.text.y = element_text(size=16), axis.title = element_text(size=24))+scale_color_manual(values = c("Amoxicillin" = "#E31A1C", "Placebo" = "#1F78B4"))+scale_x_discrete(limits=c("SAM_Amoxicillin_wk0","SAM_Amoxicillin_wk1","SAM_Amoxicillin_wk4","SAM_Amoxicillin_wk8","SAM_Amoxicillin_wk12","SAM_Amoxicillin_wk104","SAM_Placebo_wk0","SAM_Placebo_wk1","SAM_Placebo_wk4","SAM_Placebo_wk8","SAM_Placebo_wk12","SAM_Placebo_wk104"))
ggsave(file="2021Figs/230122_AmoxPlaBCV2.pdf")
pairwise.wilcox.test(BrayDFamoxpla$V2, BrayDFamoxpla$Description, p.adjust.method = "BH")
#                       SAM_Amoxicillin_wk0 SAM_Amoxicillin_wk1 SAM_Amoxicillin_wk104 SAM_Amoxicillin_wk12 SAM_Amoxicillin_wk4 SAM_Amoxicillin_wk8 SAM_Placebo_wk0 SAM_Placebo_wk1 SAM_Placebo_wk104
# SAM_Amoxicillin_wk1   0.06921             -                   -                     -                    -                   -                   -               -               -                
# SAM_Amoxicillin_wk104 0.35825             0.00295             -                     -                    -                   -                   -               -               -                
# SAM_Amoxicillin_wk12  0.38101             0.00555             0.99430               -                    -                   -                   -               -               -                
# SAM_Amoxicillin_wk4   0.15234             0.00034             0.67259               0.72247              -                   -                   -               -               -                
# SAM_Amoxicillin_wk8   0.02050             0.00018             0.11425               0.30399              0.52382             -                   -               -               -                
# SAM_Placebo_wk0       0.74899             0.04090             0.77134               0.65325              0.35825             0.13351             -               -               -                
# SAM_Placebo_wk1       0.54171             0.01148             0.81264               0.80723              0.54171             0.14527             0.80723         -               -                
# SAM_Placebo_wk104     0.28683             0.01148             0.74899               0.79202              0.97829             0.54171             0.64546         0.71465         -                
# SAM_Placebo_wk12      0.13284             0.00034             0.54171               0.66617              0.91839             0.62495             0.30399         0.54171         0.87125          
# SAM_Placebo_wk4       0.01148             1.5e-06             0.14527               0.35640              0.54171             0.82107             0.06921         0.13182         0.62495          
# SAM_Placebo_wk8       0.13284             0.00181             0.54171               0.72247              0.91857             0.54171             0.35825         0.54171         0.87125          
#                       SAM_Placebo_wk12 SAM_Placebo_wk4
# SAM_Amoxicillin_wk1   -                -              
# SAM_Amoxicillin_wk104 -                -              
# SAM_Amoxicillin_wk12  -                -              
...
# SAM_Placebo_wk104     -                -              
# SAM_Placebo_wk12      -                -              
# SAM_Placebo_wk4       0.68382          -              
# SAM_Placebo_wk8       0.94249          0.66617     
...
ggMarginal(p, type="boxplot", groupColour = TRUE, size=5)
ggsave(file="2021Figs/230522_Week104BrayPCOAwboxplot.pdf")

adonis2(Bray104~Treatment, data = Bray104meta, permutations = 999, method = "bray") 
 
#          Df SumOfSqs      R2      F Pr(>F)
# Treatment  3    26762 0.28292 9.8636  0.528
# Residual  75    67829 0.71708              
# Total     78    94591 1.00000         
...
#To ask about compositional differences between baseline amox/pla and the followup SAM and non-sam kids
baselineandreference<-subset(widespeciesmeta, Week_of_follow_up=="0"|Description=="Healthy_NA_wk104"|Description=="SAMcontrol_NA_wk104")
.rowNamesDF(baselineandreference)<-baselineandreference$Sample
baselineandreferenceBray<-baselineandreference[-c(1:212)]
baselineandreferenceBraypco <-vegdist(baselineandreferenceBray, method = "bray")
pco.baselineandreferenceBraypco<-pco(baselineandreferenceBraypco, k=4)
baselineandreferenceBray<-as.data.frame(pco.baselineandreferenceBraypco$points)
### Eigenvalues for all data##
EigenbaselineandreferenceBray<- rbind(SD = sqrt(pco.baselineandreferenceBraypco$eig),Proportion = pco.baselineandreferenceBraypco$eig/sum(pco.baselineandreferenceBraypco$eig),Cumulative = cumsum(pco.baselineandreferenceBraypco$eig)/sum(pco.baselineandreferenceBraypco$eig))
#V1 = 27.5 and V2 = 12.1

baselineandreferenceBray$Sample<-rownames(baselineandreferenceBray)
baselineandreferenceBraymeta<-join(baselineandreferenceBray, metadata)
baselineandreferenceBray$Sample=NULL

baselineandreferenceBraymeta$Description=factor(baselineandreferenceBraymeta$Description, levels = c("SAM_Placebo_wk0","SAM_Amoxicillin_wk0","Healthy_NA_wk104","SAMcontrol_NA_wk104"))

q<-ggplot(baselineandreferenceBraymeta, aes(x=V1, y=V2, color=Treatment)) +geom_point(size=5)+
  theme_bw()+theme(legend.text=element_text(size=12, face="bold"),legend.title=element_text(size=14, face="bold"),
        panel.grid.minor = element_blank(), panel.border = element_rect(colour = "black"), 
        axis.text.x= element_text(size=12,angle=45,hjust=1,vjust=1,face="bold"),
        axis.text.y= element_text(size=12,face="bold"))+ scale_color_manual(values = c("Amoxicillin" = "#E31A1C", "Placebo" = "#1F78B4","SAMcontrol" = "#FF7F00", "Healthy" = "#33A02C"))+stat_ellipse(geom='path', position = 'identity',type = 't', level=0.95)+
  ...
  xlab("PCO1 (27.5%)")
ggMarginal(q, type="boxplot", groupColour = TRUE, size=5)
ggsave(file="2021Figs/230522_baselineandrefBrayPCOAwboxplot.pdf")

adonis2(baselineandreferenceBray~Treatment, data = baselineandreferenceBraymeta, permutations = 999, method = "bray") 
#           Df SumOfSqs      R2      F Pr(>F)  
# Treatment   3   147652 0.89737 311.87  0.067 .
# Residual  107    16886 0.10263                
# Total     110   164539 1.00000                

kruskal.test(V1~Treatment, data = baselineandreferenceBraymeta)
# Kruskal-Wallis chi-squared = 3.2786, df = 3, p-value = 0.3506
pairwise.wilcox.test(baselineandreferenceBraymeta$V1, baselineandreferenceBraymeta$Treatment, p.adjust.method = "BH")
#            Amoxicillin Healthy Placebo
# Healthy    0.38        -       -      
# Placebo    0.94        0.38    -      
# SAMcontrol 0.38        0.38    0.38 

kruskal.test(V2~Treatment, data = baselineandreferenceBraymeta)
# Kruskal-Wallis chi-squared = 4.9402, df = 3, p-value = 0.1762
pairwise.wilcox.test(baselineandreferenceBraymeta$V2, baselineandreferenceBraymeta$Treatment, p.adjust.method = "BH")
#            Amoxicillin Healthy Placebo
# Healthy    0.13        -       -      
# Placebo    0.62        0.62    -      
# SAMcontrol 0.62        0.88    0.88   
```
...
#Richness/diversity on humann3 data
Humann3_KEGG$shannon<-diversity(Humann3_KEGG, index="shannon")
Humann3_KEGG$richness<-specnumber(Humann3_KEGG)
Humann3_metacyc$shannon<-diversity(Humann3_metacyc, index="shannon")
Humann3_metacyc$richness<-specnumber(Humann3_metacyc)
Humann3_pathwayabundanceShannon<-diversity(Humann3_pathwayabundance, index="shannon")
Humann3_pathwayabundancerichness<-specnumber(Humann3_pathwayabundance)
Humann3_pathwayabundance$Shannon<-Humann3_pathwayabundanceShannon
Humann3_pathwayabundance$richness<-Humann3_pathwayabundancerichness

Humann3_KEGG$Sample<-row.names(Humann3_KEGG)
Humann3_metacyc$Sample<-row.names(Humann3_metacyc)
Humann3_pathwayabundance$Sample<-row.names(Humann3_pathwayabundance)
HumannKeggmeta3<-join(Humann3_KEGG, metadata, by = "Sample")
HumannMetacycmeta3<-join(Humann3_metacyc, metadata, by = "Sample")
Humannpathwayabundancemeta3<-join(metadata,Humann3_pathwayabundance, by = "Sample")

fitshanndescriptionHumannKEGG<- lmer(Shannon~Description+ (1|idno), data=Humannpathwayabundancemeta3)
anova(fitshanndescriptionHumannKEGG)
fitshanndescriptionHumannKEGG<-lsmeans(fitshanndescriptionHumannKEGG, pairwise~Description, adjust="tukey")
fitshanndescriptionHumannKEGG

 # contrast                                      estimate     SE  df t.ratio p.value
 # Healthy_NA_wk104 - SAM_Amoxicillin_wk0        3.24e-02 0.0225 406   1.437  0.9770
 # Healthy_NA_wk104 - SAM_Amoxicillin_wk1        3.86e-02 0.0220 404   1.759  0.8926
 # Healthy_NA_wk104 - SAM_Amoxicillin_wk104      3.98e-02 0.0256 414   1.553  0.9566
 # Healthy_NA_wk104 - SAM_Amoxicillin_wk12       3.80e-02 0.0217 403   1.753  0.8951
 # Healthy_NA_wk104 - SAM_Amoxicillin_wk4        2.82e-02 0.0225 407   1.253  0.9933
 # Healthy_NA_wk104 - SAM_Amoxicillin_wk8        6.08e-02 0.0235 411   2.592  0.3491
 # Healthy_NA_wk104 - SAM_Placebo_wk0            5.42e-02 0.0231 407   2.347  0.5214
 # Healthy_NA_wk104 - SAM_Placebo_wk1            4.19e-02 0.0229 408   1.829  0.8617
 # Healthy_NA_wk104 - SAM_Placebo_wk104          1.95e-03 0.0306 413   0.064  1.0000
 # Healthy_NA_wk104 - SAM_Placebo_wk12           2.82e-02 0.0227 407   1.242  0.9938
 # Healthy_NA_wk104 - SAM_Placebo_wk4            2.32e-02 0.0215 403   1.082  0.9984
 # Healthy_NA_wk104 - SAM_Placebo_wk8            5.39e-02 0.0242 412   2.231  0.6074
 # Healthy_NA_wk104 - SAMcontrol_NA_wk104        1.18e-02 0.0425 390   0.279  1.0000
 # SAM_Amoxicillin_wk0 - SAM_Amoxicillin_wk1     6.23e-03 0.0210 306   0.297  1.0000
 # SAM_Amoxicillin_wk0 - SAM_Amoxicillin_wk104   7.40e-03 0.0243 281   0.304  1.0000
 # SAM_Amoxicillin_wk0 - SAM_Amoxicillin_wk12    5.64e-03 0.0210 339   0.268  1.0000
 # SAM_Amoxicillin_wk0 - SAM_Amoxicillin_wk4    -4.15e-03 0.0217 322  -0.192  1.0000
 # SAM_Amoxicillin_wk0 - SAM_Amoxicillin_wk8     2.84e-02 0.0225 310   1.261  0.9927
 # SAM_Amoxicillin_wk0 - SAM_Placebo_wk0         2.18e-02 0.0234 414   0.929  0.9997
 # SAM_Amoxicillin_wk0 - SAM_Placebo_wk1         9.47e-03 0.0232 414   0.408  1.0000
 # SAM_Amoxicillin_wk0 - SAM_Placebo_wk104      -3.04e-02 0.0309 409  -0.985  0.9994
 # SAM_Amoxicillin_wk0 - SAM_Placebo_wk12       -4.17e-03 0.0231 414  -0.181  1.0000
 # SAM_Amoxicillin_wk0 - SAM_Placebo_wk4        -9.18e-03 0.0218 413  -0.421  1.0000
 # SAM_Amoxicillin_wk0 - SAM_Placebo_wk8         2.15e-02 0.0245 414   0.878  0.9998
 # SAM_Amoxicillin_wk0 - SAMcontrol_NA_wk104    -2.05e-02 0.0427 395  -0.481  1.0000
 # SAM_Amoxicillin_wk1 - SAM_Amoxicillin_wk104   1.17e-03 0.0242 320   0.048  1.0000
 # SAM_Amoxicillin_wk1 - SAM_Amoxicillin_wk12   -5.92e-04 0.0205 351  -0.029  1.0000
 # SAM_Amoxicillin_wk1 - SAM_Amoxicillin_wk4    -1.04e-02 0.0212 337  -0.489  1.0000
 # SAM_Amoxicillin_wk1 - SAM_Amoxicillin_wk8     2.21e-02 0.0220 320   1.005  0.9993
 # SAM_Amoxicillin_wk1 - SAM_Placebo_wk0         1.55e-02 0.0229 413   0.680  1.0000
 # SAM_Amoxicillin_wk1 - SAM_Placebo_wk1         3.24e-03 0.0227 414   0.143  1.0000
 # SAM_Amoxicillin_wk1 - SAM_Placebo_wk104      -3.67e-02 0.0305 409  -1.203  0.9954
 # SAM_Amoxicillin_wk1 - SAM_Placebo_wk12       -1.04e-02 0.0225 413  -0.463  1.0000
 # SAM_Amoxicillin_wk1 - SAM_Placebo_wk4        -1.54e-02 0.0212 412  -0.727  1.0000
 # SAM_Amoxicillin_wk1 - SAM_Placebo_wk8         1.53e-02 0.0239 414   0.637  1.0000
 # SAM_Amoxicillin_wk1 - SAMcontrol_NA_wk104    -2.68e-02 0.0424 394  -0.632  1.0000
 # SAM_Amoxicillin_wk104 - SAM_Amoxicillin_wk12 -1.76e-03 0.0242 336  -0.073  1.0000
 # SAM_Amoxicillin_wk104 - SAM_Amoxicillin_wk4  -1.16e-02 0.0249 328  -0.465  1.0000
 # SAM_Amoxicillin_wk104 - SAM_Amoxicillin_wk8   2.10e-02 0.0255 316   0.822  0.9999
 # SAM_Amoxicillin_wk104 - SAM_Placebo_wk0       1.44e-02 0.0264 412   0.545  1.0000
 # SAM_Amoxicillin_wk104 - SAM_Placebo_wk1       2.07e-03 0.0262 412   0.079  1.0000
 # SAM_Amoxicillin_wk104 - SAM_Placebo_wk104    -3.78e-02 0.0332 405  -1.139  0.9973
 # SAM_Amoxicillin_wk104 - SAM_Placebo_wk12     -1.16e-02 0.0261 412  -0.444  1.0000
 # SAM_Amoxicillin_wk104 - SAM_Placebo_wk4      -1.66e-02 0.0250 413  -0.664  1.0000
 # SAM_Amoxicillin_wk104 - SAM_Placebo_wk8       1.41e-02 0.0273 410   0.515  1.0000
 # SAM_Amoxicillin_wk104 - SAMcontrol_NA_wk104  -2.79e-02 0.0444 402  -0.630  1.0000
 # SAM_Amoxicillin_wk12 - SAM_Amoxicillin_wk4   -9.80e-03 0.0210 341  -0.466  1.0000
 # SAM_Amoxicillin_wk12 - SAM_Amoxicillin_wk8    2.27e-02 0.0217 307   1.050  0.9988
 # SAM_Amoxicillin_wk12 - SAM_Placebo_wk0        1.61e-02 0.0226 413   0.714  1.0000
 # SAM_Amoxicillin_wk12 - SAM_Placebo_wk1        3.83e-03 0.0224 413   0.171  1.0000
 # SAM_Amoxicillin_wk12 - SAM_Placebo_wk104     -3.61e-02 0.0303 409  -1.191  0.9958
 # SAM_Amoxicillin_wk12 - SAM_Placebo_wk12      -9.81e-03 0.0222 413  -0.441  1.0000
 # SAM_Amoxicillin_wk12 - SAM_Placebo_wk4       -1.48e-02 0.0209 412  -0.708  1.0000
 # SAM_Amoxicillin_wk12 - SAM_Placebo_wk8        1.58e-02 0.0237 414   0.669  1.0000
 # SAM_Amoxicillin_wk12 - SAMcontrol_NA_wk104   -2.62e-02 0.0422 394  -0.620  1.0000
 # SAM_Amoxicillin_wk4 - SAM_Amoxicillin_wk8     3.25e-02 0.0225 307   1.449  0.9751
 # SAM_Amoxicillin_wk4 - SAM_Placebo_wk0         2.59e-02 0.0234 414   1.107  0.9980
 # SAM_Amoxicillin_wk4 - SAM_Placebo_wk1         1.36e-02 0.0232 414   0.587  1.0000
 # SAM_Amoxicillin_wk4 - SAM_Placebo_wk104      -2.63e-02 0.0309 408  -0.851  0.9999
 # SAM_Amoxicillin_wk4 - SAM_Placebo_wk12       -1.94e-05 0.0231 414  -0.001  1.0000
 # SAM_Amoxicillin_wk4 - SAM_Placebo_wk4        -5.03e-03 0.0218 413  -0.230  1.0000
 # SAM_Amoxicillin_wk4 - SAM_Placebo_wk8         2.56e-02 0.0245 414   1.048  0.9989
 # SAM_Amoxicillin_wk4 - SAMcontrol_NA_wk104    -1.64e-02 0.0427 395  -0.384  1.0000
 # SAM_Amoxicillin_wk8 - SAM_Placebo_wk0        -6.61e-03 0.0243 414  -0.272  1.0000
 # SAM_Amoxicillin_wk8 - SAM_Placebo_wk1        -1.89e-02 0.0241 414  -0.784  1.0000
 # SAM_Amoxicillin_wk8 - SAM_Placebo_wk104      -5.88e-02 0.0316 407  -1.863  0.8449
 # SAM_Amoxicillin_wk8 - SAM_Placebo_wk12       -3.26e-02 0.0239 414  -1.359  0.9858
 # SAM_Amoxicillin_wk8 - SAM_Placebo_wk4        -3.76e-02 0.0228 414  -1.650  0.9314
 # SAM_Amoxicillin_wk8 - SAM_Placebo_wk8        -6.89e-03 0.0253 413  -0.272  1.0000
 # SAM_Amoxicillin_wk8 - SAMcontrol_NA_wk104    -4.89e-02 0.0432 398  -1.134  0.9974
 # SAM_Placebo_wk0 - SAM_Placebo_wk1            -1.23e-02 0.0226 337  -0.544  1.0000
 # SAM_Placebo_wk0 - SAM_Placebo_wk104          -5.22e-02 0.0298 313  -1.750  0.8961
 # SAM_Placebo_wk0 - SAM_Placebo_wk12           -2.59e-02 0.0229 383  -1.132  0.9975
 # SAM_Placebo_wk0 - SAM_Placebo_wk4            -3.10e-02 0.0216 371  -1.433  0.9775
 # SAM_Placebo_wk0 - SAM_Placebo_wk8            -2.89e-04 0.0242 368  -0.012  1.0000
 # SAM_Placebo_wk0 - SAMcontrol_NA_wk104        -4.23e-02 0.0430 396  -0.985  0.9994
 # SAM_Placebo_wk1 - SAM_Placebo_wk104          -3.99e-02 0.0302 350  -1.320  0.9890
 # SAM_Placebo_wk1 - SAM_Placebo_wk12           -1.36e-02 0.0226 371  -0.604  1.0000
 # SAM_Placebo_wk1 - SAM_Placebo_wk4            -1.87e-02 0.0214 373  -0.870  0.9998
 # SAM_Placebo_wk1 - SAM_Placebo_wk8             1.20e-02 0.0239 355   0.504  1.0000
 # SAM_Placebo_wk1 - SAMcontrol_NA_wk104        -3.00e-02 0.0429 396  -0.700  1.0000
 # SAM_Placebo_wk104 - SAM_Placebo_wk12          2.63e-02 0.0305 381   0.860  0.9999
 # SAM_Placebo_wk104 - SAM_Placebo_wk4           2.13e-02 0.0295 373   0.720  1.0000
 # SAM_Placebo_wk104 - SAM_Placebo_wk8           5.19e-02 0.0315 372   1.647  0.9321
 # SAM_Placebo_wk104 - SAMcontrol_NA_wk104       9.89e-03 0.0475 408   0.208  1.0000
 # SAM_Placebo_wk12 - SAM_Placebo_wk4           -5.01e-03 0.0211 363  -0.237  1.0000
 # SAM_Placebo_wk12 - SAM_Placebo_wk8            2.57e-02 0.0235 341   1.090  0.9983
 # SAM_Placebo_wk12 - SAMcontrol_NA_wk104       -1.64e-02 0.0428 395  -0.383  1.0000
 # SAM_Placebo_wk4 - SAM_Placebo_wk8             3.07e-02 0.0224 334   1.370  0.9846
 # SAM_Placebo_wk4 - SAMcontrol_NA_wk104        -1.14e-02 0.0421 393  -0.270  1.0000
 # SAM_Placebo_wk8 - SAMcontrol_NA_wk104        -4.20e-02 0.0435 399  -0.965  0.9995

fitrichdescriptionHumannKEGG<- lmer(richness~Description+ (1|idno), data=Humannpathwayabundancemeta3)
anova(fitrichdescriptionHumannKEGG)
fitrichdescriptionHumannKEGG<-lsmeans(fitrichdescriptionHumannKEGG, pairwise~Description, adjust="tukey")
fitrichdescriptionHumannKEGG
# 
# 
#  Description           lsmean    SE  df lower.CL upper.CL
#  Healthy_NA_wk104         278  9.45 412      259      296
#  SAM_Amoxicillin_wk0      302  9.86 414      283      321
#  SAM_Amoxicillin_wk1      316  9.34 414      298      334
#  SAM_Amoxicillin_wk104    261 12.44 413      236      285
#  SAM_Amoxicillin_wk12     288  9.10 414      270      306
#  SAM_Amoxicillin_wk4      275  9.86 414      256      294
#  SAM_Amoxicillin_wk8      276 10.65 414      255      297
#  SAM_Placebo_wk0          259 10.31 414      238      279
#  SAM_Placebo_wk1          278 10.15 414      258      298
#  SAM_Placebo_wk104        276 16.19 413      244      308
#  SAM_Placebo_wk12         281 10.00 414      262      301
#  SAM_Placebo_wk4          277  8.89 414      259      294
#  SAM_Placebo_wk8          274 11.23 414      252      296
#  SAMcontrol_NA_wk104      309 23.77 412      262      356
# 
# Degrees-of-freedom method: kenward-roger 
# Confidence level used: 0.95 
# 
# $contrasts
#  contrast                                     estimate   SE  df t.ratio p.value
#  Healthy_NA_wk104 - SAM_Amoxicillin_wk0        -24.091 13.7 413  -1.765  0.8905
#  Healthy_NA_wk104 - SAM_Amoxicillin_wk1        -38.246 13.3 413  -2.880  0.1908
#  Healthy_NA_wk104 - SAM_Amoxicillin_wk104       16.976 15.6 414   1.087  0.9983
#  Healthy_NA_wk104 - SAM_Amoxicillin_wk12       -10.119 13.1 413  -0.771  1.0000
#  Healthy_NA_wk104 - SAM_Amoxicillin_wk4          3.007 13.7 414   0.220  1.0000
#  Healthy_NA_wk104 - SAM_Amoxicillin_wk8          1.833 14.2 414   0.129  1.0000
#  Healthy_NA_wk104 - SAM_Placebo_wk0             19.183 14.0 414   1.372  0.9846
#  Healthy_NA_wk104 - SAM_Placebo_wk1             -0.446 13.9 414  -0.032  1.0000
#  Healthy_NA_wk104 - SAM_Placebo_wk104            2.082 18.7 414   0.111  1.0000
#  Healthy_NA_wk104 - SAM_Placebo_wk12            -3.569 13.8 414  -0.259  1.0000
#  Healthy_NA_wk104 - SAM_Placebo_wk4              1.184 13.0 413   0.091  1.0000
#  Healthy_NA_wk104 - SAM_Placebo_wk8              3.957 14.7 414   0.270  1.0000
#  Healthy_NA_wk104 - SAMcontrol_NA_wk104        -30.939 25.6 412  -1.209  0.9952
#  SAM_Amoxicillin_wk0 - SAM_Amoxicillin_wk1     -14.154 13.4 327  -1.060  0.9987
#  SAM_Amoxicillin_wk0 - SAM_Amoxicillin_wk104    41.068 15.6 313   2.633  0.3238
#  SAM_Amoxicillin_wk0 - SAM_Amoxicillin_wk12     13.973 13.3 359   1.054  0.9988
#  SAM_Amoxicillin_wk0 - SAM_Amoxicillin_wk4      27.099 13.7 344   1.971  0.7844
#  SAM_Amoxicillin_wk0 - SAM_Amoxicillin_wk8      25.925 14.3 339   1.812  0.8693
#  SAM_Amoxicillin_wk0 - SAM_Placebo_wk0          43.275 14.3 414   3.034  0.1305
#  SAM_Amoxicillin_wk0 - SAM_Placebo_wk1          23.645 14.1 414   1.671  0.9249
#  SAM_Amoxicillin_wk0 - SAM_Placebo_wk104        26.174 19.0 414   1.381  0.9837
#  SAM_Amoxicillin_wk0 - SAM_Placebo_wk12         20.523 14.0 414   1.462  0.9735
#  SAM_Amoxicillin_wk0 - SAM_Placebo_wk4          25.276 13.3 414   1.904  0.8233
#  SAM_Amoxicillin_wk0 - SAM_Placebo_wk8          28.048 14.9 414   1.877  0.8377
#  SAM_Amoxicillin_wk0 - SAMcontrol_NA_wk104      -6.847 25.7 413  -0.266  1.0000
#  SAM_Amoxicillin_wk1 - SAM_Amoxicillin_wk104    55.222 15.4 352   3.594  0.0250
#  SAM_Amoxicillin_wk1 - SAM_Amoxicillin_wk12     28.127 12.9 368   2.180  0.6441
#  SAM_Amoxicillin_wk1 - SAM_Amoxicillin_wk4      41.253 13.4 356   3.077  0.1175
#  SAM_Amoxicillin_wk1 - SAM_Amoxicillin_wk8      40.079 14.0 347   2.868  0.1967
#  SAM_Amoxicillin_wk1 - SAM_Placebo_wk0          57.429 13.9 414   4.129  0.0035
#  SAM_Amoxicillin_wk1 - SAM_Placebo_wk1          37.799 13.8 414   2.741  0.2598
#  SAM_Amoxicillin_wk1 - SAM_Placebo_wk104        40.328 18.7 414   2.158  0.6604
#  SAM_Amoxicillin_wk1 - SAM_Placebo_wk12         34.677 13.7 414   2.535  0.3868
#  SAM_Amoxicillin_wk1 - SAM_Placebo_wk4          39.430 12.9 414   3.059  0.1225
#  SAM_Amoxicillin_wk1 - SAM_Placebo_wk8          42.203 14.6 414   2.890  0.1861
#  SAM_Amoxicillin_wk1 - SAMcontrol_NA_wk104       7.307 25.5 413   0.286  1.0000
#  SAM_Amoxicillin_wk104 - SAM_Amoxicillin_wk12  -27.095 15.3 366  -1.776  0.8856
#  SAM_Amoxicillin_wk104 - SAM_Amoxicillin_wk4   -13.969 15.7 362  -0.890  0.9998
#  SAM_Amoxicillin_wk104 - SAM_Amoxicillin_wk8   -15.143 16.2 353  -0.936  0.9997
#  SAM_Amoxicillin_wk104 - SAM_Placebo_wk0         2.207 16.2 414   0.137  1.0000
#  SAM_Amoxicillin_wk104 - SAM_Placebo_wk1       -17.423 16.1 414  -1.085  0.9984
#  SAM_Amoxicillin_wk104 - SAM_Placebo_wk104     -14.894 20.4 413  -0.729  1.0000
#  SAM_Amoxicillin_wk104 - SAM_Placebo_wk12      -20.545 16.0 414  -1.287  0.9913
#  SAM_Amoxicillin_wk104 - SAM_Placebo_wk4       -15.792 15.3 414  -1.033  0.9990
#  SAM_Amoxicillin_wk104 - SAM_Placebo_wk8       -13.020 16.8 414  -0.777  1.0000
#  SAM_Amoxicillin_wk104 - SAMcontrol_NA_wk104   -47.915 26.8 413  -1.786  0.8816
#  SAM_Amoxicillin_wk12 - SAM_Amoxicillin_wk4     13.126 13.3 359   0.990  0.9994
#  SAM_Amoxicillin_wk12 - SAM_Amoxicillin_wk8     11.952 13.8 331   0.867  0.9999
#  SAM_Amoxicillin_wk12 - SAM_Placebo_wk0         29.302 13.8 414   2.130  0.6798
#  SAM_Amoxicillin_wk12 - SAM_Placebo_wk1          9.672 13.6 414   0.709  1.0000
#  SAM_Amoxicillin_wk12 - SAM_Placebo_wk104       12.201 18.6 414   0.657  1.0000
#  SAM_Amoxicillin_wk12 - SAM_Placebo_wk12         6.550 13.5 414   0.484  1.0000
#  SAM_Amoxicillin_wk12 - SAM_Placebo_wk4         11.303 12.7 414   0.888  0.9998
#  SAM_Amoxicillin_wk12 - SAM_Placebo_wk8         14.076 14.5 414   0.974  0.9995
#  SAM_Amoxicillin_wk12 - SAMcontrol_NA_wk104    -20.820 25.5 413  -0.818  0.9999
#  SAM_Amoxicillin_wk4 - SAM_Amoxicillin_wk8      -1.174 14.3 334  -0.082  1.0000
#  SAM_Amoxicillin_wk4 - SAM_Placebo_wk0          16.176 14.3 414   1.134  0.9974
#  SAM_Amoxicillin_wk4 - SAM_Placebo_wk1          -3.454 14.2 414  -0.244  1.0000
#  SAM_Amoxicillin_wk4 - SAM_Placebo_wk104        -0.925 19.0 414  -0.049  1.0000
#  SAM_Amoxicillin_wk4 - SAM_Placebo_wk12         -6.576 14.0 414  -0.468  1.0000
#  SAM_Amoxicillin_wk4 - SAM_Placebo_wk4          -1.823 13.3 414  -0.137  1.0000
#  SAM_Amoxicillin_wk4 - SAM_Placebo_wk8           0.949 14.9 414   0.064  1.0000
#  SAM_Amoxicillin_wk4 - SAMcontrol_NA_wk104     -33.946 25.7 413  -1.319  0.9892
#  SAM_Amoxicillin_wk8 - SAM_Placebo_wk0          17.350 14.8 414   1.171  0.9965
#  SAM_Amoxicillin_wk8 - SAM_Placebo_wk1          -2.279 14.7 414  -0.155  1.0000
#  SAM_Amoxicillin_wk8 - SAM_Placebo_wk104         0.249 19.4 413   0.013  1.0000
#  SAM_Amoxicillin_wk8 - SAM_Placebo_wk12         -5.402 14.6 414  -0.370  1.0000
#  SAM_Amoxicillin_wk8 - SAM_Placebo_wk4          -0.649 13.9 414  -0.047  1.0000
#  SAM_Amoxicillin_wk8 - SAM_Placebo_wk8           2.124 15.5 414   0.137  1.0000
#  SAM_Amoxicillin_wk8 - SAMcontrol_NA_wk104     -32.772 26.1 413  -1.258  0.9930
#  SAM_Placebo_wk0 - SAM_Placebo_wk1             -19.630 14.3 354  -1.375  0.9842
#  SAM_Placebo_wk0 - SAM_Placebo_wk104           -17.101 18.9 348  -0.903  0.9998
#  SAM_Placebo_wk0 - SAM_Placebo_wk12            -22.752 14.3 392  -1.596  0.9466
#  SAM_Placebo_wk0 - SAM_Placebo_wk4             -17.999 13.5 383  -1.334  0.9880
#  SAM_Placebo_wk0 - SAM_Placebo_wk8             -15.227 15.1 386  -1.007  0.9992
#  SAM_Placebo_wk0 - SAMcontrol_NA_wk104         -50.122 25.9 413  -1.934  0.8065
#  SAM_Placebo_wk1 - SAM_Placebo_wk104             2.529 19.0 383   0.133  1.0000
#  SAM_Placebo_wk1 - SAM_Placebo_wk12             -3.123 14.1 381  -0.221  1.0000
#  SAM_Placebo_wk1 - SAM_Placebo_wk4               1.631 13.4 384   0.122  1.0000
#  SAM_Placebo_wk1 - SAM_Placebo_wk8               4.403 15.0 372   0.294  1.0000
#  SAM_Placebo_wk1 - SAMcontrol_NA_wk104         -30.492 25.9 413  -1.180  0.9962
#  SAM_Placebo_wk104 - SAM_Placebo_wk12           -5.651 19.0 404  -0.298  1.0000
#  SAM_Placebo_wk104 - SAM_Placebo_wk4            -0.898 18.4 399  -0.049  1.0000
#  SAM_Placebo_wk104 - SAM_Placebo_wk8             1.875 19.6 400   0.096  1.0000
#  SAM_Placebo_wk104 - SAMcontrol_NA_wk104       -33.021 28.8 414  -1.148  0.9971
#  SAM_Placebo_wk12 - SAM_Placebo_wk4              4.753 13.2 374   0.359  1.0000
#  SAM_Placebo_wk12 - SAM_Placebo_wk8              7.526 14.8 358   0.507  1.0000
#  SAM_Placebo_wk12 - SAMcontrol_NA_wk104        -27.370 25.8 413  -1.061  0.9987
#  SAM_Placebo_wk4 - SAM_Placebo_wk8               2.773 14.1 355   0.196  1.0000
#  SAM_Placebo_wk4 - SAMcontrol_NA_wk104         -32.123 25.4 413  -1.266  0.9926
#  SAM_Placebo_wk8 - SAMcontrol_NA_wk104         -34.895 26.3 413  -1.327  0.9885
# 
# Degrees-of-freedom method: kenward-roger 
# P value adjustment: tukey method for comparing a family of 14 estimates 

#Richness over time Humann3 KEGG
ggplot(HumannKeggmeta3, aes(x=as.factor(Week_of_follow_up), y=richness, color=Treatment)) + geom_boxplot(outlier.shape = NA) +geom_point(position = position_jitterdodge(jitter.width = 0.1)) +ggtitle("Richness")+xlab("Timepoint")+ylab("Number of KEGG pathways") + theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme(axis.text.x = element_text(size=16), axis.title.x=element_text(size=20), axis.title.y = element_text(size=20), axis.text.y = element_text(size=16), axis.title = element_text(size=24))+scale_color_manual(values = c("Amoxicillin" = "#E31A1C", "Placebo" = "#1F78B4","SAMcontrol" = "#FF7F00", "Healthy" = "#33A02C"))
ggsave(file="2021Figs/220120_Humann3KEGGovertime.pdf")

#Plotting Shannon Diversity over time Humann3 KEGG
ggplot(HumannKeggmeta3, aes(x=as.factor(Week_of_follow_up), y=shannon, color=Treatment)) + geom_boxplot(outlier.shape = NA) +geom_point(position = position_jitterdodge(jitter.width = 0.1)) +ggtitle("Shannon Diversity")+xlab("Timepoint")+ylab("Shannon Diversity of KEGG pathways") + theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme(axis.text.x = element_text(size=16), axis.title.x=element_text(size=20), axis.title.y = element_text(size=20), axis.text.y = element_text(size=16), axis.title = element_text(size=24))+scale_color_manual(values = c("Amoxicillin" = "#E31A1C", "Placebo" = "#1F78B4","SAMcontrol" = "#FF7F00", "Healthy" = "#33A02C"))
ggsave(file="2021Figs/220120_Humann3KEGGShannonDiversityovertime.pdf")

#Richness over time Humann3 metacyc
ggplot(HumannMetacycmeta3, aes(x=as.factor(Week_of_follow_up), y=richness, color=Treatment)) + geom_boxplot(outlier.shape = NA) +geom_point(position = position_jitterdodge(jitter.width = 0.1)) +ggtitle("Richness")+xlab("Timepoint")+ylab("Number of metacyc pathways")+ theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme(axis.text.x = element_text(size=16), axis.title.x=element_text(size=20), axis.title.y = element_text(size=20), axis.text.y = element_text(size=16), axis.title = element_text(size=24))+scale_color_manual(values = c("Amoxicillin" = "#E31A1C", "Placebo" = "#1F78B4","SAMcontrol" = "#FF7F00", "Healthy" = "#33A02C"))
ggsave(file="2021Figs/220120_Humann3metacycovertime.pdf")

#Plotting Shannon Diversity over time metacyc
ggplot(HumannMetacycmeta3, aes(x=as.factor(Week_of_follow_up), y=shannon, color=Treatment)) + geom_boxplot(outlier.shape = NA) +geom_point(position = position_jitterdodge(jitter.width = 0.1)) +ggtitle("Shannon Diversity")+xlab("Timepoint")+ylab("Shannon Diversity of Metacyc pathways") + theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme(axis.text.x = element_text(size=16), axis.title.x=element_text(size=20), axis.title.y = element_text(size=20), axis.text.y = element_text(size=16), axis.title = element_text(size=24))+scale_color_manual(values = c("Amoxicillin" = "#E31A1C", "Placebo" = "#1F78B4","SAMcontrol" = "#FF7F00", "Healthy" = "#33A02C"))
ggsave(file="2021Figs/220120_Humann3metacycShannonDiversityovertime.pdf")

#Richness over time Humann3 Pathway abundance
ggplot(Humannpathwayabundancemeta3, aes(x=as.factor(Week_of_follow_up), y=richness, color=Treatment)) + geom_boxplot(outlier.shape = NA) +geom_point(position = position_jitterdodge(jitter.width = 0.1)) +ggtitle("Richness")+xlab("Timepoint")+ylab("Number of pathways")+ theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme(axis.text.x = element_text(size=16), axis.title.x=element_text(size=20), axis.title.y = element_text(size=20), axis.text.y = element_text(size=16), axis.title = element_text(size=24))+scale_color_manual(values = c("Amoxicillin" = "#E31A1C", "Placebo" = "#1F78B4","SAMcontrol" = "#FF7F00", "Healthy" = "#33A02C"))
ggsave(file="2021Figs/220608_Humann3pathwayabundancerichnessovertime.pdf")

#Plotting Shannon Diversity over time Pathway abundance
ggplot(Humannpathwayabundancemeta3, aes(x=as.factor(Week_of_follow_up), y=Shannon, color=Treatment)) + geom_boxplot(outlier.shape = NA) +geom_point(position = position_jitterdodge(jitter.width = 0.1)) +ggtitle("Shannon Diversity")+xlab("Timepoint")+ylab("Shannon Diversity of pathways") + theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme(axis.text.x = element_text(size=16), axis.title.x=element_text(size=20), axis.title.y = element_text(size=20), axis.text.y = element_text(size=16), axis.title = element_text(size=24))+scale_color_manual(values = c("Amoxicillin" = "#E31A1C", "Placebo" = "#1F78B4","SAMcontrol" = "#FF7F00", "Healthy" = "#33A02C"))
ggsave(file="2021Figs/220608_Humann3PathwayabundanceShannonDiversityovertime.pdf")
```
...
#Log transformed for reviewer?
ggplot(Genuswk0to1, aes(x=as.factor(Week_of_follow_up), y=g__Lactobacillus, color=Treatment)) + geom_boxplot(outlier.shape = NA) +geom_point(position = position_jitterdodge(jitter.width = 0.1)) +ggtitle("Lactobacillus")+xlab("Timepoint")+ylab("Lactobacillus abundance") + theme(axis.text.x = element_text(angle = 45, hjust = 1))+theme(axis.text.x = element_text(size=16), axis.title.x=element_text(size=20), axis.title.y = element_text(size=20), axis.text.y = element_text(size=16), axis.title = element_text(size=24))+scale_color_manual(values = c("Amoxicillin" = "#E31A1C", "Placebo" = "#1F78B4"))+scale_y_log10()
...
#Stats on week 104 phylum
pairwise.wilcox.test(week104phylum$p__Actinobacteria, week104phylum$Description, p.adjust.method = "BH")
